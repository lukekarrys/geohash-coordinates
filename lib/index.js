...
var getDaysAfter = function getDaysAfter(date, days) {
  return formatDate(_moment2['default'](date).add({ days: days }));
};
...
var geohashCoordinates = function geohashCoordinates(_x3, cb) {
  ...
  _async.parallel([fetchEast ? eastDecimals : passThrough, fetchWest ? westDecimals : passThrough], function (err, results) {
    if (err) return cb(err);
...
var latest = function latest(options, cb) {
  var optsWithDefaults = _lodashObjectDefaults2['default'](options, {
    date: formatDate(),
    days: 4
  });
  ...
  var dates = _lodashUtilityRange2['default'](days).map(function (index) {
    return getDaysAfter(startDate, index);
  });

  _async.mapSeries(dates, function (date, cb) {
    geohashCoordinates(_lodashObjectAssign2['default']({}, _lodashObjectOmit2['default'](options, 'days'), { date: date }), function (err, result) {
      cb(err, result ? _lodashObjectAssign2['default']({}, result, { date: date }) : null);
    });
    ...
    var validResults = results && _lodashArrayCompact2['default'](results);
